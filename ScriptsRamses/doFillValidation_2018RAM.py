...
# the next.
#
# Modified version for validating 2018 fills for RAMSES.
...
luminometers = ['bcm1f', 'ramses', 'hfoc', 'pltzero']

# luminometers to actually update the file for
luminometers_out = ['ramses']

# Default priority order for luminometers.
defaultLumiPriority = ['hfoc', 'ramses']
...
primaryLuminometers = ['hfoc18v6', 'ramses18v1']

# Datatag to be used for each luminometer.
datatags = {'bcm1f': 'bcm1f18v4',
            'hfoc': 'hfoc18v6',
            'ramses': 'ramses18v1',
            'pltzero': 'pltzero18v3Reproc'}
...
emailTargets = {'bcm1f': 'bcm1f', 'hfoc': 'hf', 'pltzero': 'pltzero', 'ramses': 'ramses'}
...
summaryEmailRecipients = ['david.peter.stickland@cern.ch', 'anne.evelyn.dabrowski@cern.ch', 'paul.lujan@cern.ch'] 

# Paths to various things.
lumiValidatePath = "./lumiValidate_2018RAM.py"      # script for making fill validation plot
fillListFile = "./fills_2018.txt"                   # list of 2018 fills
logFileName = "./fillValidationLog_2018RAM.json"    # log JSON
lumiJSONFileNamePattern = "./normtag_%s_2018.json"  # filename pattern for individual luminometer JSONs
...
class InvalidateDialog:
    def __init__(self, parent):
        ...
        self.selectedLumin.set(luminometers_out[0])
        self.luminMenu = apply(OptionMenu, (self.dwin, self.selectedLumin)+tuple(luminometers_out))
...
def displayPlot():
    ...
    cmd = "python "+lumiValidatePath+" -f "+str(fillNumber)+" -b \"STABLE BEAMS\" --normtag "+" ".join([datatags[i] for i in luminometers])
...
def gitCommit():
    ...
    commitFiles = [logFileName]
    for l in luminometers_out:
...
def makeEmails():
    ...
    defaultEmailBody = "Hello,\n\nThis is an automated email to let you know that the RAMSES fill validation was performed for the following fill"+suffix+" by "+userName+":\n"
...
def getValidSections(fillNumber, l, referenceLumi):
    ...
    with open(tempFileName) as csv_input:
        ...
        for row in reader:
            ...
            if (thisdet.lower() != l and not (l == "pcc" and thisdet.lower() == "pxl")):
...
def produceOutput():
    ...
    # No need to create the best lumi file for this purpose
...
# Next read the list of fills and populate the fill list.
fillList = []
fillFile = open(fillListFile, "r")
allFills = fillFile.readlines()
fillFile.close()
for i in allFills:
    if int(i) > lastFill:
        fillList.append(int(i))
nfills = len(fillList)
