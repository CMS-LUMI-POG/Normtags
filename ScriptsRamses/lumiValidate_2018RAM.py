...
FIGURE_ADJUSTS_TWO_FAT_ROWS = {
    "top": 0.96, "left": 0.06, "right": 0.80, "bottom": 0.15, "hspace": 0.55}
FIGURE_ADJUSTS_TWO_FAT_ONE_SLIM_ROWS = {
    "top": 0.96, "left": 0.06, "right": 0.80, "bottom": 0.04, "hspace": 0.75}
XAXIS_TICKS_MAX = 40
...
def get_data(types, normtags, run=None, fill=None, beams=None):
    ...
    cmd_tpl = prepare_brilcalc_call_tpl(run, fill, beams)
    got_cols = []
    merged = None
    for request in types + normtags:
        ...
        # Phew...this is a bit of a dirty hack here to get the base luminometer name
        # from the normtag
        lumi_name = request
        index_pos = request.find("18")
        if index_pos >= 0:
            lumi_name = request[0:index_pos]
        
        file_name = "temp_"+lumi_name+".csv"
        cmd += ["-o", file_name]
        if not os.path.exists(file_name):
            ...
            log.debug("using existing file: %s", file_name)
        ...
        with open(file_name) as inputFile:
            ...
            for row in reader:
                if row[0][0] != '#':
                    ...
                    if (lumi_name == "dt" and run not in seenRuns):
            ...
            outfile.close()
        os.system("mv tmp_output.csv "+file_name)

        data = pandas.read_csv(file_name, skiprows=1)
        data = data[:-3]
        if data.empty:
...
def get_bunch_data(run=None, fill=None, beams=None):
    ...
    cmd = prepare_brilcalc_call_tpl(run, fill, beams)
    cmd += ["--xing"]
    log.debug("calling subprocess: %s", " ".join(cmd))
    # (Jonas) TODO: check return code
    subprocess.call(cmd)
